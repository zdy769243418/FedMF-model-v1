...
item_id_list = [int(e[0]) for e in item_frequent_dict[:num_items]]  # 拿到打分用户数量最多的前40部电影的id ，列表形式保存

user_id_list = sorted(set([e[0] for e in ratings]), key=lambda x:int(x))[:num_users]  # 按用户id从小到大，取前10个用户的
